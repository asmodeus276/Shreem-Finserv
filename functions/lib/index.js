...
exports.trackApplication = exports.submitLead = void 0;
...
exports.submitLead = (0, https_1.onCall)({
    ...
    if (!data) {
        throw new https_1.HttpsError("invalid-argument", "Missing application payload.");
    }
    ...
    if (!consent) {
        throw new https_1.HttpsError("failed-precondition", "Mandatory DPDP consent for data sharing with lending partners is required to submit an application.");
    }
    // 2. Input Validation
    if (!fullName || fullName.length < 2) {
        throw new https_1.HttpsError("invalid-argument", "Please enter your full name as per PAN card.");
    }
    if (cleanMobile.length !== 10) {
        throw new https_1.HttpsError("invalid-argument", "Please enter a valid 10-digit Indian mobile number.");
    }
    if (!city) {
        throw new https_1.HttpsError("invalid-argument", "Please provide your current city.");
    }
 ...
 */
exports.trackApplication = (0, https_1.onCall)({
    region: "asia-south1",
    cors: true,
    maxInstances: 10,
}, async (request) => {
    ...
    if (!appId) {
        throw new https_1.HttpsError("invalid-argument", "Please provide an Application Reference ID.");
    }
    const docRef = db.collection("leads").doc(appId);
    const docSnap = await docRef.get();
    if (!docSnap.exists) {
        throw new https_1.HttpsError("not-found", "Application ID not found. Please verify your reference number or contact support.");
    }
